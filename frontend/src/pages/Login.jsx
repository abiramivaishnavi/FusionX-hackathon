      ...
      if (isLogin) {
        ...
        if (pendingCredential) {
          try {
            ...
            // Optionally, we could show a toast here, but the login succeeded so we proceed
          }
        }
      } else {
      ...
      if (err.code === 'auth/account-exists-with-different-credential') {
        ...
        try {
          ...
          if (existingMethod === 'google.com') {
            ...
            // We can't auto-link (either password or email enumeration protection is ON)
            setPendingCredential(pendingCred);
            setFormData(prev => ({ ...prev, email }));
            setIsLogin(true); // Switch to login tab
            
            setError(`An account already exists for ${email}. Please click "Google" below or enter your password above to link your accounts.`);
            setIsLoading(false);
            return;
          }
